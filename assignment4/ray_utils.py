...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    idx = random.sample(range(total_pixels), n_pixels)
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    cam_points = camera.unproject_points(ndc_points, from_ndc = True)

    # TODO (1.3): Get ray origins from camera center
    rays_o = camera.get_camera_center().expand(ndc_points.shape[0],-1)

    # TODO (1.3): Get normalized ray directions
    rays_d = F.normalize(cam_points - rays_o, dim=-1)
